...
def pseudo_fluctuation_measure(eval, evec, K = 10, funct = (lambda x: 1/x)):
    ...
    Arguments:
        ...
        K : int - number of modes to be considered, excluding first trivial mode
    ...
    """
    # First mode is related to translation
    eval = eval[1:K+1]
...
def plot_flexibility_value(id, cutoff, k = 10, flex: Literal["msqf", "bfact", "pseudo"] = "msqf"):
    ...
    if flex == "msqf":
        ...
        ax.set_xlabel("Amino Acid Unit", size=16)
        ax.set_ylabel("Mean-Squared Fluctuation", size=16)
        ...
        ax.set_xlabel("Amino Acid Unit", size=16)
        ax.set_ylabel("Pseudo Fluctuation", size=16)
    else:
